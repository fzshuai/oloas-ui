  ...
  methods: {
    resetTaskForm() {
      ...
      if (this.dataType === "USERS") {
        let userIdData =
          bpmnElementObj["assignee"] || bpmnElementObj["candidateUsers"];
    ...
    handleTaskUserComplete() {
      ...
      this.selectedUser.text =
        this.selectedUserDate.map((k) => k.nickName) || [];
      if (this.selectedUserDate.length === 1) {
        ...
        userTaskForm.candidateUsers =
          this.selectedUserDate.map((k) => k.userId).join() || null;
        userTaskForm.text =
          this.selectedUserDate.map((k) => k.nickName).join() || null;
    ...
    checkedDeptChange(checkedIds, checkedData) {
      ...
      if (checkedData && checkedData.length > 0) {
        userTaskForm.candidateGroups =
          checkedData.map((k) => k.id).join() || null;
    ...
    changeDataType(val) {
      ...
      if (val === "USERS") {
        if (
          this.selectedUser &&
          this.selectedUser.ids &&
          this.selectedUser.ids.length > 0
        ) {
