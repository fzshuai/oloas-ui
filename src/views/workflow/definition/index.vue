  ...
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="流程分类" align="center" prop="categoryName">
        <template slot-scope="scope">
          <span>{{
            categoryOptions.find((k) => k.code === scope.row.category)
              .categoryName
          }}</span>
        </template>
      </el-table-column>
    ...
    >
      ...
      <div slot="footer" class="dialog-footer">
        <el-button type="primary" @click="handleLoadXml(process.form)"
          >确 定</el-button
        >
...
export default {
  ...
  // 解决go(-1)导致不刷新原界面问题，但不太可行，每次切换页面都会调用
  // watch: {
  //   $route() {
  //     console.log("go(-1)");
  //     this.getList();
